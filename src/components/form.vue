	...
	<section id="application" class="section">
		...
		<div class="section__form">
			...
			<button
				...
				:disabled="GET_FINISH_AMOUNT || loadingButton ? false : true"
...
export default {
	...
	computed: {
		...mapGetters({
			GET_FINISH_AMOUNT: 'businessCalc/GET_FINISH_AMOUNT',
		}),
	},
	mounted() {
	...
	methods: {
		...mapActions({
			SEND_APPLICATION: 'businessCalc/SEND_APPLICATION',
		}),
		onlyNumbers(event) {
			const regexp = /[0-9]/i
		...
		async sendPost() {
			if (!this.checkErrors()) {
				const data = {
					...
					return_url: 'https://vsk.bexram.online/shield',
					fail_url: 'https://vsk.bexram.online/',
				}
				this.loadingButton = true
				const res = await this.SEND_APPLICATION(data)
				if (res.status === 200) {
					window.open(res.data, '_blank', false)
				} else {
